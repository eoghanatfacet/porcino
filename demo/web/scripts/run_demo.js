...
const rhinoModelName = `rhino_params${suffix}.pv`;
fs.copyFileSync(
  path.join(modelDir, rhinoModelName),
  path.join(outputDirectory, rhinoModelName),
);

fs.writeFileSync(
  path.join(outputDirectory, "rhinoModel.js"),
  `const rhinoModel = {
  publicPath: "models/${rhinoModelName}",
  customWritePath: "${version}_${rhinoModelName}",
};
